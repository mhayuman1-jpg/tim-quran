...
async function getProfil() {
  try {
    ...
    if (error) {
      console.error('[GaleriLayout] Supabase error:', error.message);
      return null;
    }

    return data ?? null;
  } catch (error) {
    console.error('[GaleriLayout] Fetch error:', error);
    return null;
  }
}

export default async function GaleriLayout({ children }: { children: ReactNode }) {
  const profil = await getProfil();

  return (
    <div className="min-h-dvh flex flex-col bg-amber-50 text-slate-800">
      <PublicNavbar

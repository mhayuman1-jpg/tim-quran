...
async function getProfil() {
  try {
    ...
    if (error) {
      console.error('[PengumumanLayout] Supabase error:', error.message);
      return null;
    }

    return data ?? null;
  } catch (error) {
    console.error('[PengumumanLayout] Fetch error:', error);
    return null;
  }
}
      ...
      <footer className="bg-amber-50">
        <div className="max-w-6xl mx-auto px-6 py-8">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
            ...
            <a href="/" className="text-xs text-slate-500 hover:text-amber-600 transition-colors">
              ← Kembali ke Beranda
            </a>
          </div>
        </div>

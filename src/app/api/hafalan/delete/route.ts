// src/app/api/hafalan/delete/route.ts
// DELETE: Hapus satu catatan hafalan berdasarkan id
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    let query = supabase.from('hafalan').delete().eq('id', id);
    ...
    if (error) {
      console.error('Hafalan delete error:', error);
      return NextResponse.json({ message: 'Gagal menghapus catatan hafalan.', error: error.message }, { status: 500 });
    }

    return NextResponse.json({ message: 'Catatan hafalan berhasil dihapus.' }, { status: 200 });
  } catch (err) {
    console.error('[hafalan delete]', err);
